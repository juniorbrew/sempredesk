      ...
      <div style={{ background:'#fff', borderRadius:16, width:'100%', maxWidth:520, boxShadow:'0 20px 60px rgba(0,0,0,0.3)', overflow:'hidden' }}>
        ...
        <div style={{ background:'#1E293B', padding:'10px 22px', display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ width:32, height:32, borderRadius:8, background:'#334155', display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:700, color:'#94A3B8', flexShrink:0 }}>
            {customerName(ticket.clientId, ticket.contactName, ticket.clientName).slice(0,2).toUpperCase()}
          </div>
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ fontSize:11, color:'#64748B' }}>{customerName(ticket.clientId, ticket.contactName, ticket.clientName)}{ticket.department ? ` · ${ticket.department}` : ''}</div>
          </div>
              ...
              {cards.map(t => {
                ...
                return (
                  <div key={t.id}
                    ...
                    {/* Client */}
                    <div style={{ fontSize:11, color:'#5E6C84', marginBottom:3, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{customerName(t.clientId, t.contactName, t.clientName)}</div>
            ...
            <div style={{ flex:1, overflowY:'auto' }}>
              {loading ? (
                ...
                <div key={t.id}
                  ...
                  <div style={{ padding:'11px 8px', fontSize:11, color:S.txt2, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{customerName(t.clientId, t.contactName, t.clientName)}</div>
                  <div style={{ padding:'11px 8px' }}>
