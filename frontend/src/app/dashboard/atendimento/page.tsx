              ...
              const renderItem = (c: any) => {
                ...
                return (
                  <ConversationListItem
                    key={c.id}
                    contactName={dispName}
                    initialsText={dispName !== '—' ? initials(dispName) : ''}
                    avatarBg={col}
                    channel={ch}
                    timeLabel={timeAgo(c.lastMessageAt || c.createdAt)}
                    preview={preview}
                    ticketNumber={!noTicket && c.ticketNumber ? String(c.ticketNumber) : null}
                    companyName={compName ? String(compName) : null}
                    isClosed={isClo}
                    ...
                    onSelect={() => {
                      setSelected(c);
                      if (c?.id) setUnreadCounts((p) => { const n = { ...p }; delete n[c.id]; return n; });
                    }}
                  />
                );
              };
