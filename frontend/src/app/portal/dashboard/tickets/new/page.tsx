...
export default function PortalNewTicketPage() {
  ...
  const selectedDept = useMemo(
    () => departments.find((d: any) => d.name === form.department),
    [departments, form.department],
  );
  const categories = selectedDept?.categories || [];
  const selectedCat = useMemo(
    () => categories.find((c: any) => c.name === form.category),
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.error?.message || data?.message || 'Erro ao criar ticket');
      }

      const ticketRef = data?.data?.ticket_id || data?.ticket_id || data?.data?.id || data?.id;
      router.push(`/portal/dashboard/tickets/${encodeURIComponent(ticketRef)}`);
    } catch (e: any) {
      ...
      <form onSubmit={onSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 16, padding: 24 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              <label style={lbl}>
                Assunto <span style={{ color: '#6366F1' }}>*</span>
              </label>
              <input
                value={form.subject}
            ...
            <div>
              <label style={lbl}>
                Descrição <span style={{ color: '#6366F1' }}>*</span>
              </label>
              <textarea
                ...
                required
                rows={5}
                placeholder="Detalhe o problema, mensagens de erro e passos para reproduzir."
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <Sel
                ...
                onChange={(e: any) => setForm({ ...form, department: e.target.value, category: '', subcategory: '' })}
              >
              ...
              {form.department && (
                <Sel
                  ...
                  onChange={(e: any) => setForm({ ...form, category: e.target.value, subcategory: '' })}
                >
                  <option value="">Selecione</option>
              ...
              {form.category && (
                <Sel
                  ...
                  onChange={(e: any) => setForm({ ...form, subcategory: e.target.value })}
                >
                  <option value="">Selecione</option>
