...
import { Ticket, LayoutDashboard, FileText, BookOpen, User, LogOut, ArrowLeftRight } from 'lucide-react';

const ChatWidget = dynamic(() => import('@/components/portal/ChatWidget'), { ssr: false });

const NAV = [
  { href:'/portal/dashboard',           icon:LayoutDashboard, label:'Início' },
  { href:'/portal/dashboard/tickets',   icon:Ticket,          label:'Meus Tickets' },
  { href:'/portal/dashboard/contracts', icon:FileText,        label:'Contratos' },
  { href:'/portal/dashboard/knowledge', icon:BookOpen,        label:'Base de Conhecimento' },
  { href:'/portal/dashboard/profile',   icon:User,            label:'Meu Perfil' },
];
...
export default function PortalDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!client) { router.replace('/portal/select-company'); }
  }, [accessToken, client]);

  const logout = () => { clearAuth(); router.push('/portal/login'); };
  const isActive = (href: string) => href === '/portal/dashboard' ? pathname === href : pathname.startsWith(href);
  ...
  return (
    <div style={{ display:'flex', height:'100vh', background:S.bg2, fontFamily:'Inter,sans-serif', overflow:'hidden' }}>
      {/* Sidebar */}
      <aside style={{ width:220, flexShrink:0, background:S.bg, borderRight:`1px solid ${S.bd}`, display:'flex', flexDirection:'column', height:'100%' }}>
        ...
        {(contact || client) && (
          <div style={{ padding:'12px 16px', borderBottom:`1px solid ${S.bd}` }}>
            ...
            {/* Trocar empresa */}
            {clients.length > 1 && (
              <button onClick={() => router.push('/portal/select-company')}
                style={{ display:'flex', alignItems:'center', gap:7, width:'100%', marginTop:10, padding:'6px 10px', background:S.accentL, border:`1px solid ${S.accentM}`, borderRadius:8, color:S.accent, fontSize:11, fontWeight:600, cursor:'pointer', transition:'all 0.15s' }}
                onMouseEnter={e=>e.currentTarget.style.background=S.accentM}
                onMouseLeave={e=>e.currentTarget.style.background=S.accentL}>
                <ArrowLeftRight style={{ width:12, height:12, flexShrink:0 }} />
                <span>Trocar empresa</span>
                <span style={{ marginLeft:'auto', background:S.accentM, color:S.accent, padding:'1px 6px', borderRadius:20, fontSize:10, fontWeight:700 }}>{clients.length}</span>
              </button>
            )}
          </div>
        )}
